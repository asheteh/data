...
def Apni_To_Jaise_Taise__Housefull__O_Dhanno_Sargam_Notes_for_Harmonium_Piano(request):
...
def _Ek_Pardesi_Mera_Dil_Le_Gaya_Harmonium_Sargam__Piano_Notes(request):
	text = Sargams.objects.filter(id__iexact =82)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Rashke_Qamar__Baadshaho__Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =83)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Rashke_Qamar_Piano_Keyboard_Notations__CDF_Notes_(request):
	text = Sargams.objects.filter(id__iexact =86)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aap_Ki_Nazron_Ne_Smjha_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =180)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mile_Ho_Tum_Humko_Piano_Notes_in_English__CDF_Notations_(request):
	text = Sargams.objects.filter(id__iexact =197)
	lists = Sargams.objects.filter(song_names__icontains='p')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Teri_Deewani_Kailash_Kher_Harmonium_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =201)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Teri_Deewani_Kailash_Kher_Piano_Notes_in_CDF_Notations(request):
	text = Sargams.objects.filter(id__iexact =203)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ghanshyam_Teri_Bansi_Harmonium_Sargam__Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =220)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Krishna_Bhajan_Mohan_Se_Dil_Kyun_Lagaya_Hai_Notes_Sargam(request):
	text = Sargams.objects.filter(id__iexact =221)
	lists = Sargams.objects.filter(song_names__icontains='a')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Om_Jai_Jagdish_Hare_Aarti_Harmonium_Piano_Notes_in_Hindi__sargam_(request):
	text = Sargams.objects.filter(id__iexact =222)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Sanwali_Surat_Pe_Mohan_Dil_Deewana_Hogya_Sargam_Notes(request):
	text = Sargams.objects.filter(id__iexact =223)
	lists = Sargams.objects.filter(song_names__icontains='a')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Om_Jai_Jagdish_Hare_Aarti_Harmonium_Notes__Slow_Tutorial_(request):
	text = Sargams.objects.filter(id__iexact =224)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Jai_Ganesh_Jai_Ganesh_Jai_Ganesh_Deva_Harmonium_Notes__Sargam(request):
	text = Sargams.objects.filter(id__iexact =225)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Gayati_Mantra_Piano_Notes_and_Harmonium_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =226)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Om_Jai_Jagdish_Hare_Aarti_Piano_Notes_and_Tutorial(request):
	text = Sargams.objects.filter(id__iexact =227)
	lists = Sargams.objects.filter(song_names__icontains='p')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kabhi_Ram_Banke_Kabhi_Shyam_Banke_Bhajan_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =228)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dar_Chhod_Ke_Jaunga_Na_Maiya_Bhajan_Sargam__Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =229)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Hey_Ram_Hey_Ram_Bhajan_Sargam_for_Harmonium__Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =230)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Radhe_Radhe_Japo_Chale_Ayenge_Bihari_Sargam_Harmonium__Piano_Notes(request):
	text = Sargams.objects.filter(id__iexact =231)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Yashomati_Maiya_Se_Bole_Nandlala_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =232)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Gaiya_Maiya_Bula_Rahi_Hai_Harmonium_Sargam__Piano_Notes(request):
	text = Sargams.objects.filter(id__iexact =233)
	lists = Sargams.objects.filter(song_names__icontains='a')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aa_Jao_Bhole_Baba_Mere_Makan_Mein_Sargam_Harmonium__Piano_Notes(request):
	text = Sargams.objects.filter(id__iexact =234)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Jana_Gana_Mana_National_Anthem_Harmonium_Notes_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =239)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kala_Kala_Kahe_Gujri_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =240)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Sare_Jahan_Se_Acha_Hindustan_Humara_Harmonium_Notes__Piano_Notations(request):
	text = Sargams.objects.filter(id__iexact =241)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Vande_Mataram_Song_Notes_for_Piano_and_Harmonium(request):
	text = Sargams.objects.filter(id__iexact =243)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Hum_Honge_Kamyab_Song_Harmonium_Notations__Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =244)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Chupke_Chupke_Raat_Din_Ghulam_Ali_Ghazal_Notes_Sargam(request):
	text = Sargams.objects.filter(id__iexact =249)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Hum_Tere_Shahar_Mein_Aaye_Hain__Ghazal__Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =250)
	lists = Sargams.objects.filter(song_names__icontains='p')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Rashke_Qamar_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =257)
	lists = Sargams.objects.filter(song_names__icontains='p')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aaarti_Kunj_Bihari_ki_Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =262)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Bahut_Pyaar_Karte_Hain_Tumko_Sanam_Notes_Sargam(request):
	text = Sargams.objects.filter(id__iexact =267)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kitna_Haseen_Chehra__Dilwale__Song_Notes__Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =268)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ajeeb_Dastan_Hai_Yeh_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =269)
	lists = Sargams.objects.filter(song_names__icontains='a')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Abhi_Mujh_Mein_Kahin__Sonu_Nigam__Harmonium_Notes__Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =270)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Tumhe_Dillagi_Bhool_Jani_Padegi_Harmonium_Sargam__Piano_Notes_in_Hindi_(request):
	text = Sargams.objects.filter(id__iexact =272)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mile_Ho_Tum_Humko_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =273)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Baharon_Phool_Barsao_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =275)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Abhi_Mujh_Mein_Kahin_Piano_Notes_in_CDF_Notations(request):
	text = Sargams.objects.filter(id__iexact =276)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Likhe_Jo_Khat_Tujhe_Harmonium_Sargam___Piano_Notes(request):
	text = Sargams.objects.filter(id__iexact =277)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ye_Reshmi_Zulfein_Ye_Sharbati_Aankhein_Notes_Sargam(request):
	text = Sargams.objects.filter(id__iexact =278)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aane_Se_Uske_Aaye_Bahar_Notes_Sargam_in_Hindi_for_Harmonium_and_Piano(request):
	text = Sargams.objects.filter(id__iexact =279)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Sapno_Ki_Rani_Kab_Aayegi_Tu_Harmonium_Sagam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =280)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aaj_Purani_Raahon_Se_Koi_Mujhe__Song__Harmonium_Sargam__Notes_Piano(request):
	text = Sargams.objects.filter(id__iexact =281)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Milti_Hai_Zindagi_Mein_Mohabbat_Kabhi_Kabhi_Harmonium_Notes_Notations_Sargam(request):
	text = Sargams.objects.filter(id__iexact =282)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aye_Mere_Watan_Ke_Logo_Piano_Harmonium_Notes(request):
	text = Sargams.objects.filter(id__iexact =283)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Desh_Ki_Dharti_Notes_for_Piano_and_Harmonium(request):
	text = Sargams.objects.filter(id__iexact =284)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Itna_Na_Mujhse_Tu_Pyar_Badha_Piano_Notes_in_Hindi__Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =285)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kajra_Mohabbat_Wala_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =286)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Mehboob_Qayamat_Hogi_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =287)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dil_Ke_Jharoke_Mein_Piano_Notes_in_Hindi__Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =288)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Lag_Jaa_Gale_Song_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =289)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Saamne_Wale_Khidki_Song_Sargam__Notations_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =290)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Sanam_Re_Full_Song_Piano_Notes__Line_by_Line_By_Notations_with_Lyrics(request):
	text = Sargams.objects.filter(id__iexact =294)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kaun_Tujhe__MS_Dhoni__Piano_Notes__Notations_with_Lyrics(request):
	text = Sargams.objects.filter(id__iexact =295)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mann_Dole_Mera_Tann_Dole_Sargam_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =297)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Hai_Apna_Dil_To_Awara_Piano_Notes_in_Hindi__Harmonium_Sargam_(request):
	text = Sargams.objects.filter(id__iexact =298)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kya_Khoob_Lagti_Ho_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =300)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Gulabi_Aankhein_Jo_Teri_Dekhi_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =301)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ek_Pyar_Ka_Nagma_Hai_Harmonium_Notes__Sargam__Sa_Re_Ga_Ma_Notations(request):
	text = Sargams.objects.filter(id__iexact =302)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dil_Lootne_Wale_Jadugar_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =303)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Bachpan_Ki_Mohabbat_Ko__Baiju_Bawra__Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =304)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ghar_Aaya_Mera_Pardesi_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =305)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Yeh_Shaam_Mastani_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =306)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Bahon_Mein_Chale_Aao_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =308)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Chalte_Chalte_Yunhi_Koi_Sargam__Harmonium_Notes__Piano_Notation(request):
	text = Sargams.objects.filter(id__iexact =309)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ek_Pyar_Ka_Nagma_Hai__Shor__keyboard_Piano_Notations_Notes(request):
	text = Sargams.objects.filter(id__iexact =311)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Tere_Bina_Zindagi_Se_Koi_Shikwa_Notes_in_Hindi__Sargam__Piano_Notations(request):
	text = Sargams.objects.filter(id__iexact =312)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Zindagi_Ek_Safar_Hai_Suhana_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =313)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Main_Shayar_To_Nahin_Harmoinim_Notes__Sargam_in_Hindi_for_Piano(request):
	text = Sargams.objects.filter(id__iexact =314)
	lists = Sargams.objects.filter(song_names__icontains='t')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Tujhe_Dekha_To_Ye_Jaana_Sanam_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =316)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Bahut_Pyar_Karte_Hain_Piano_Notes(request):
	text = Sargams.objects.filter(id__iexact =317)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Phoolon_Sa_Chehera_Tera_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =319)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Phoolon_Sa_Chehra_Tera_Song_Piano_Notes__CDF_Notations(request):
	text = Sargams.objects.filter(id__iexact =320)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Tum_To_Thehre_Pardesi__Altaf_Raja__Harmonium_Notes__Piano_Notations(request):
	text = Sargams.objects.filter(id__iexact =322)
	lists = Sargams.objects.filter(song_names__icontains='b')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Didi_Tera_Devar_Deewana_Sargam__Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =323)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Bahut_Pyar_Karte_Hain_Tumko_Sanam_Harmonium_Sargam__Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =326)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dil_Ke_Armaan_Aansuon_Mein_Beh_Gaye_Notes_Sargam(request):
	text = Sargams.objects.filter(id__iexact =327)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Pal_Pal_Dil_Ke_Paas_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =328)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Hum_Tumhe_Chahte_Hain_Aise_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =330)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Teri_Hai_Zameen_Tera_Aasman_Harmonium_Notes(request):
	text = Sargams.objects.filter(id__iexact =332)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ek_Do_Teen__Old_Song__Piano_Notes__CDF_Notations_for_Keyboard(request):
	text = Sargams.objects.filter(id__iexact =333)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Main_Teri_Dushman_Song_Piano_Notes_in_Hindi__Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =334)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kaun_Disa_Mein_Leke_Piano_Notes_in_Hindi__Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =335)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Neele_Neele_Ambar_Par_song_Piano_Notes_in_Hindi__Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =337)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Darde_Dil_Darde_Jigar__Karz__Piano_Notes_in_Hindi__Harmonium_Sargam(request):
	text = Sargams.objects.filter(id__iexact =338)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Hothon_Se_Chhulo_Tum_Song_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =339)
	lists = Sargams.objects.filter(song_names__icontains='c')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Teri_Mitti__Kesari__Song_Harmonium_Sargam__Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =340)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Teri_Mitti__Kesari__Piano_Notes__Western_Notations(request):
	text = Sargams.objects.filter(id__iexact =341)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dheere_Dheere_Se_Meri_Zindagi_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =343)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Apni_to_Jaise_Taise_Song_Piano_Notes__Notations_(request):
	text = Sargams.objects.filter(id__iexact =345)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mera_Dil_Bhi_Kitna_Pagal_Hai_Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =346)
	lists = Sargams.objects.filter(song_names__icontains='a')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ek_Do_Teen__Tezaab__Song_Harmonium_Sargam__Piano_Notes_in_Hindi_(request):
	text = Sargams.objects.filter(id__iexact =347)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dil_Hai_Ke_Manta_Nahi_Harmonium_Notes__Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =348)
	lists = Sargams.objects.filter(song_names__icontains='e')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aye_Mere_Humsafar_Ek_Zara_Intezaar_Harmonium_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =349)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Dheere_Dheere_Se_Meri_Zindagi_Mein_Piano_Keyboard_Notes_in_CDF(request):
	text = Sargams.objects.filter(id__iexact =350)
	lists = Sargams.objects.filter(song_names__icontains='lo')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Aaye_Ho_Meri_Zindagi_Mein__Raja_Hindustani__Notes_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =351)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Sandese_Aate_Hain_Piano_Notes_in_Sa_re_ga_ma(request):
	text = Sargams.objects.filter(id__iexact =352)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Kaun_Tujhe__MS_Dhoni__Harmonium_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =353)
	lists = Sargams.objects.filter(song_names__icontains='pyar')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Moh_Moh_Ke_Dhaage_Song_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =354)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Pehla_Nasha_Pehla_Khumar_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =356)
	lists = Sargams.objects.filter(song_names__icontains='m')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Rang_Barse_Bheege_Chunar_Wali_Harmonium_Notes_Sargam_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =357)
	lists = Sargams.objects.filter(song_names__icontains='d')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Pardesi_Pardesi_Jana_Nahi_Notes__Sargam_in_Hindi_for_Harmonium_and_Piano(request):
	text = Sargams.objects.filter(id__iexact =358)
	lists = Sargams.objects.filter(song_names__icontains='n')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Mere_Naseeb_Mein_Tu_Hai_Ke_Nahin_Sargam__Piano_Notes_in_Hindi(request):
	text = Sargams.objects.filter(id__iexact =359)
	lists = Sargams.objects.filter(song_names__icontains='l')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Ab_To_Hai_Tumse_Song_Sargam__Piano_Notes_in_Hindi_(request):
	text = Sargams.objects.filter(id__iexact =360)
	lists = Sargams.objects.filter(song_names__icontains='p')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Do_Lafzon_Ki_Hai_Dil_Ki_Kahani_Sargam_Notes(request):
	text = Sargams.objects.filter(id__iexact =361)
	lists = Sargams.objects.filter(song_names__icontains='s')[:10]
	context={'songs' : text,'lists':lists}
	return render(request,'music/songs.html',context)

def _Janam_Janam__Arijit_Singh__Piano_Notes__Notations_in_English(request):
...
def Ajib_dasta_hai_yeh(request):
